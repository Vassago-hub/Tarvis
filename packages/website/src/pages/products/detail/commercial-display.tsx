...
export default function CommercialDisplayDetailPage() {
	const { isZh } = useLanguage()
	const models = PRODUCT_MODELS.filter((m) => m.category === 'commercial_display')
				...
				<div className="max-w-7xl mx-auto px-5 lg:px-8 py-16 md:py-20">
					<div className="max-w-3xl">
						<p className="text-xs text-[#E60012] font-medium mb-3">
							{isZh ? '商用显示' : 'Commercial Display'}
						</p>
						<h1 className="text-3xl md:text-4xl font-bold text-gray-900 dark:text-white tracking-tight leading-tight">
							{isZh ? '商用拼接屏 / 会议平板 / 数字标牌' : 'Video Wall / Interactive Display / Digital Signage'}
						</h1>
						<p className="mt-4 text-gray-600 dark:text-gray-300 leading-relaxed">
							{isZh
								? '超窄边框拼接、4K 会议平板与交互式数字标牌，适用于会议室、展厅、指挥中心与零售场景。'
								: 'Ultra-narrow bezel video walls, 4K interactive panels and digital signage for meeting rooms, showrooms, control centers and retail.'}
						</p>
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-5">
						...
						>
							...
							<p className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed">
								{isZh ? '面向集成商、工程公司的合作与项目报价。' : 'Partnership and quotations for integrators and engineering firms.'}
							</p>
						</NavLink>
						<NavLink
						...
						>
							...
							<p className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed">
								{isZh ? '商用显示产品保修条款与服务范围。' : 'Warranty terms for commercial display products.'}
							</p>
						</NavLink>
						<NavLink
