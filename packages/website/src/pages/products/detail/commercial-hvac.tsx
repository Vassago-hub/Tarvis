...
export default function CommercialHvacDetailPage() {
	const { isZh } = useLanguage()
	const models = PRODUCT_MODELS.filter((m) => m.category === 'commercial_hvac')
				...
				<div className="max-w-7xl mx-auto px-5 lg:px-8 py-16 md:py-20">
					<div className="max-w-3xl">
						<p className="text-xs text-blue-600 dark:text-blue-400 font-medium mb-3">
							{isZh ? '商用空调' : 'Commercial HVAC'}
						</p>
						<h1 className="text-3xl md:text-4xl font-bold text-gray-900 dark:text-white tracking-tight leading-tight">
							{isZh ? '商用中央空调 / 精密空调' : 'Commercial Central AC & Precision Cooling'}
						</h1>
						<p className="mt-4 text-gray-600 dark:text-gray-300 leading-relaxed">
							{isZh
								? '多联机 VRV、风冷模块与精密空调，满足写字楼、酒店、工厂与数据中心的差异化需求。'
								: 'VRV multi-split, air-cooled modular and precision AC — meeting diverse needs across office, hotel, industrial and data center environments.'}
						</p>
				...
				<div className="max-w-7xl mx-auto px-5 lg:px-8">
					<div className="grid grid-cols-1 md:grid-cols-3 gap-5">
						...
						>
							...
							<p className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed">
								{isZh ? '商用空调产品保修条款与服务范围。' : 'Warranty terms for commercial HVAC products.'}
							</p>
						</NavLink>
						<NavLink
