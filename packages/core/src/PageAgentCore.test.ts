...
describe('PageAgentCore tool safety', () => {
	it('executes a tool after onConfirm approves a confirm decision', async () => {
		const clickElement = vi.fn(async () => ({ success: true, message: 'clicked' }))
		const pageController = {
			...
			getBrowserState: vi.fn(async () => ({
				...
				title: 'Test page',
				header: '',
				content: '[0]<button>提交预约 />',
				footer: '',
			})),
			getElementMetadata: vi.fn(async () => ({
				index: 0,
				tagName: 'button',
				text: '提交预约',
		...
		const fetchMock = createFetchMock()
			.mockResolvedValueOnce(agentResponse({ action: { click_element_by_index: { index: 0 } } }))
			.mockResolvedValueOnce(doneResponse('confirmed safely'))
		...
		const agent = createAgent(fetchMock, {
			...
			onBeforeToolExecute: () => ({
				type: 'confirm',
				riskLevel: 'high',
				reason: 'submit requires confirmation',
				userMessage: '请确认是否提交',
			}),
		})
		agent.onConfirm = onConfirm

		const result = await agent.execute('submit repair form')

		expect(result).toMatchObject({ success: true, data: 'confirmed safely' })
		expect(onConfirm).toHaveBeenCalledWith(
			expect.objectContaining({ description: '请确认是否提交', riskLevel: 'high' }),
			expect.objectContaining({ signal: expect.any(AbortSignal) })
		)
		expect(clickElement).toHaveBeenCalledWith(0)
	})

	it('blocks a tool before execution when onBeforeToolExecute returns block', async () => {
		const clickElement = vi.fn(async () => ({ success: true, message: 'clicked' }))
		const getElementMetadata = vi.fn(async () => ({
			index: 0,
			tagName: 'button',
			text: '提交预约',
			isExternalLink: false,
		...
		const fetchMock = createFetchMock()
			.mockResolvedValueOnce(agentResponse({ action: { click_element_by_index: { index: 0 } } }))
			.mockResolvedValueOnce(doneResponse('blocked safely'))
		const agent = createAgent(fetchMock, {
			pageController,
			onBeforeToolExecute: (ctx) => ({
				type: 'block',
				riskLevel: 'blocked',
				reason: `${ctx.toolName} blocked for test`,
			}),
		})

		const result = await agent.execute('submit repair form')

		expect(result).toMatchObject({ success: true, data: 'blocked safely' })
		expect(clickElement).not.toHaveBeenCalled()
		expect(getElementMetadata).toHaveBeenCalledWith(0)
	})
})
