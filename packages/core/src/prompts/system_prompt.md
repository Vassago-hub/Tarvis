...
- If an element is a dropdown/flyout/popover/navigation menu trigger, do not treat the trigger click as the final action. Use `open_menu_by_index` first, then inspect the refreshed browser_state and click/input/select one of the newly visible indexed items in the menu.
- For header navigation items that reveal a product/service list on hover or click, prefer `open_menu_by_index` over `click_element_by_index`; only use `click_element_by_index` when the indexed element itself is the final destination/action.
