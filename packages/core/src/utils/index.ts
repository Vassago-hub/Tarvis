...
const llmsTxtCache = new Map<string, { value: string | null; timestamp: number }>()
const llmsTxtInFlight = new Map<string, Promise<string | null>>()
/** 10-minute TTL. Refresh the llms.txt file if a new origin appears */
const LLMS_TXT_CACHE_TTL = 10 * 60 * 1000

/** Fetch /llms.txt for a URL's origin. Cached per origin with TTL, with concurrent-request dedup. */
export async function fetchLlmsTxt(url: string): Promise<string | null> {
	...
	if (cached && Date.now() - cached.timestamp < LLMS_TXT_CACHE_TTL) {
		return cached.value
	}

	const existing = llmsTxtInFlight.get(origin)
	...
	const promise = (async () => {
		...
		try {
			...
			llmsTxtCache.set(origin, { value: result, timestamp: Date.now() })
			llmsTxtInFlight.delete(origin)
		}
		return result
	})()

	llmsTxtInFlight.set(origin, promise)
	return promise
}
