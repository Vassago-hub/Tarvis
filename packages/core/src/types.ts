...
import type { ElementMetadata } from '@page-agent/page-controller'
...
export interface ToolExecutionContext {
	...
	/**
	 * AbortSignal shared by the enclosing task. Tools can use this to abort
	 * their work (e.g. long operations, scrolling, waiting, etc.) in a
	 * cooperative manner and surface a clean failure.
	 */
	signal: AbortSignal
}
...
export interface AgentConfig extends LLMConfig {
	...
	/**
	 * Called before a tool executes. Return a decision to allow, block, or require user confirmation.
	 * This is intended for product-specific safety policies such as sensitive form and external-link handling.
	 * @experimental
	 */
	onBeforeToolExecute?: (
		ctx: ToolExecutionContext,
		agent: PageAgentCore
	) => Promise<SafetyDecision | undefined> | SafetyDecision | undefined
	/**
...
/**
 * Telemetry event - lightweight, structured, privacy-friendly monitoring event.
 * Emitted via the `telemetry` CustomEvent on the agent instance. Use this to
 * hook into an external monitoring system (OpenTelemetry, console sink, etc.).
 *
 * @note Do NOT include raw user input or page content directly. Tool names
 * and element metadata are allowed because they are agent-managed.
 */
export type TelemetryEvent =
	| { type: 'task.started'; taskId: string; language: SupportedLanguage; pageUrl: string }
	| { type: 'task.completed'; taskId: string; success: boolean; durationMs: number; pageUrl: string }
	| { type: 'llm.invoked'; attempt: number; durationMs: number; toolCalled: string; success: boolean }
	| { type: 'llm.retry'; attempt: number; maxAttempts: number; errorMessage: string }
	| { type: 'tool.executed'; tool: string; durationMs: number; success: boolean; elementIndex?: number }
	| { type: 'tool.blocked'; tool: string; reason: string; elementIndex?: number }
	| { type: 'tool.confirmed'; tool: string; elementIndex?: number; confirmed: boolean }
	| { type: 'element.navigation'; url: string; external: boolean; opensInNewTab: boolean }
