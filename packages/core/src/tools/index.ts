...
tools.set(
	...
	tool({
		description:
			'Open a dropdown, flyout, popover, navigation submenu, combobox, or suggestion list by element index. Use this instead of click when the target element is a menu trigger and you need to select an item from the list that appears. After this action, inspect the updated browser_state and choose from the newly visible indexed items.',
		inputSchema: z.object({
			index: z.int().min(0),
			mode: z.enum(['auto', 'hover', 'click']).optional().default('auto'),
		}),
		execute: async function (this: PageAgentCore, input, { signal }) {
			const result = await this.pageController.openMenu(input.index, input.mode, signal)
			return result.message
		},
	})
)
