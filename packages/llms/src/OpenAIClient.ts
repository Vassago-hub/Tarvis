...
export class OpenAIClient implements LLMClient {
	...
	async invoke(
		...
		// Defensive schema validation: empty/malformed message arrays fail
		// fast here so downstream 400s from providers surface a clearer error.
		validateMessages(messages)
		if (!tools || Object.keys(tools).length === 0) {
			throw new InvokeError(
				InvokeErrorTypes.CONFIG_ERROR,
				'At least one tool must be configured for LLM invocation.'
			)
		}
...
/**
 * Minimal structural validation for the message array passed to invoke.
 * Ensures at least one message exists, that every message has a recognised
 * role, and that assistant/assistant-level fields aren't missing entirely
 * when required. Errors surface as CONFIG_ERROR InvokeErrors so retries
 * don't mask broken callers.
 */
function validateMessages(messages: Message[]): void {
	if (!Array.isArray(messages) || messages.length === 0) {
		throw new InvokeError(
			InvokeErrorTypes.CONFIG_ERROR,
			'LLM.invoke requires a non-empty messages array.'
		)
	}
	const ALLOWED_ROLES = new Set(['system', 'user', 'assistant', 'tool'] as const)
	for (let i = 0; i < messages.length; i++) {
		const m = messages[i]
		if (!m || typeof m !== 'object') {
			throw new InvokeError(
				InvokeErrorTypes.CONFIG_ERROR,
				`messages[${i}] is not a valid message object.`
			)
		}
		...
		if (!ALLOWED_ROLES.has(role as any)) {
			throw new InvokeError(
				InvokeErrorTypes.CONFIG_ERROR,
				`messages[${i}] has invalid role "${role}".`
			)
		}
		const content = (m as Message).content
		const toolCalls = (m as Message).tool_calls
		if (role === 'assistant' && (!content || typeof content !== 'string') && !toolCalls) {
			// Allow; the API still accepts pure tool_call assistant messages.
			// Keep this branch explicit so future extensions can tighten it.
		} else if ((role === 'system' || role === 'user') && (!content || !content.trim())) {
			console.warn(`[OpenAIClient] messages[${i}] (${role}) has empty content; continuing.`)
		}
	}
}
